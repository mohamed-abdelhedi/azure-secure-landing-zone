...
resource "azurerm_resource_group" "prod" {
  name     = "rg-landingzone-prod-${var.location}"
...
module "hub_network" {
  ...
  hub_vnet_name          = "vnet-hub-prod-${var.location}-001"
  ...
  enable_bastion     = true
  firewall_sku       = "Premium" # Premium SKU with IDPS in Deny mode
...
module "app_spoke" {
  ...
  resource_group_name      = azurerm_resource_group.prod.name
  location                 = azurerm_resource_group.prod.location
  spoke_vnet_name          = "vnet-app-prod-${var.location}-001"
...
module "data_spoke" {
  ...
  resource_group_name      = azurerm_resource_group.prod.name
  location                 = azurerm_resource_group.prod.location
  spoke_vnet_name          = "vnet-data-prod-${var.location}-001"
